...
class Account extends DbFactory
{
    ...
    {
        return self::$db->get_all("SELECT * FROM ".self::$dp."admin");
    }

    public function register($data)
    {
        $sql = "SELECT * FROM ".self::$dp."admin WHERE `username`=:username";

        $return = self::$db->get_one($sql, ['username'=>$data['username']]);

        if (!empty($return) or $return === true) return -1;

        $sql = "INSERT INTO ".self::$dp."admin (`username`,`password`,`salt`) VALUES";
    ...
    {
        $sql = "SELECT * FROM ".self::$dp."admin WHERE `username`=:username AND `password`=SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1(" . self::$db->quote($data['password']) . ")))))";
        #$sql = "SELECT * FROM ".self::$dp."admin WHERE `username`=:username AND `password`=:password";

        $admin_info = self::$db->get_one(
            $sql,
            [
        ...
        if (!empty($admin_info)) {
            $update_sql = "UPDATE ".self::$dp."admin SET `last_login_time`='{$date}' WHERE `username`=:username AND `password`=SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1(" . self::$db->quote($data['password']) . ")))))";
