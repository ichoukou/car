...
class Calendar extends DbFactory
{
    ...
    public function ajaxFindTeachingsList($data)
    {
        $params = $data['params'];

        $conditions = [];

        $sql = "SELECT teaching_id,teaching_date FROM ".self::$dp."teaching  " .
               " WHERE `deleted` = 1 ";

        if (!empty($params['year_month'])) {
            $sql .= "AND `teaching_date` LIKE :teaching_date ";
            $conditions['teaching_date'] = "%".$params['year_month']."%";
        }
        ...
        if (!empty($params['filter_baby_id'])) {
            $sql .= "AND `baby_id` = :baby_id ";
            $conditions['baby_id'] = $params['filter_baby_id'];
        }

        $sql .= " ORDER BY teaching_id DESC ";
    ...
    public function ajaxFindTeachingsListInfo($data)
    {
        $params = $data['params'];
        ...
        if (!empty($params['date_time'])) {
            $sql .= "AND t.`teaching_date` LIKE :teaching_date ";
            $conditions['teaching_date'] = "%".$params['date_time']."%";
        }
        ...
        $sql .= " ORDER BY t.teaching_id DESC ";
