...
class Settlement extends Controller
{
    public function index()
    {
        ...
        $this->data['reservations'] = M::Vender('Reservation\\Settlement', 'findReservations', ['params'=>$param]);
        $count = M::Vender('Reservation\\Settlement', 'findReservationsCount', ['params'=>$param]);

        $pagination         = new Pagination();
        $pagination->total  = $count['total'];
        $pagination->page   = $param['page'];
        $pagination->limit  = $param['limit'];
        $pagination->url    = $this->data['entrance'] . "route=Vender/Reservation/Settlement&page={page}{$this->data['url']}";
        ...
        $this->data['search_url'] = "{$this->data['entrance']}route=Vender/Reservation/Settlement";
    ...
    public function edit_reservation()
    {
        ...
        $reservation_info = M::Vender('Reservation\\Settlement', 'findReservationByReservationId', ['reservation_id'=>$reservation_id]);
        if (empty($reservation_info))
            exit(header("location:{$this->data['entrance']}route=Vender/Reservation/Settlement{$this->data['url']}"));
    ...
    public function do_edit_reservation()
    {
        if ($post = $this->validate_edit()) {
            $return = M::Vender('Reservation\\Settlement', 'editReservation', ['post'=>$post]);

            if ($return == -1)
                exit(json_encode(['status'=>-1, 'result'=>['other_error'=>'信息匹配错误，没有找到匹配的数据']], JSON_UNESCAPED_UNICODE));

            setcookie('success_info', '修改预约结算信息成功', time() + 60);

            exit(json_encode(['status'=>1, 'result'=>'修改预约结算信息成功'], JSON_UNESCAPED_UNICODE));
        }
    }
