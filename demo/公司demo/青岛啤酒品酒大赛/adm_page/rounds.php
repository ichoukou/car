...
$list = query("SELECT * FROM ".DB_PREFIX."rounds WHERE `deleted` = 1 AND `type` = '{$type}' {$and} ORDER BY create_time LIMIT {$param['start']},{$param['limit']}");
$count = query("SELECT COUNT(*) total FROM ".DB_PREFIX."rounds WHERE `deleted` = 1 {$and} AND `type` = '{$type}' ");

$pagination         = new Pagination();
$pagination->total  = $count->row['total'];
$pagination->page   = $param['page'];
$pagination->limit  = $param['limit'];
$pagination->url    = "rounds.php?page={page}{$url}";
