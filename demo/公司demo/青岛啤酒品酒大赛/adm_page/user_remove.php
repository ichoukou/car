...
if (empty($user_id)) {
    ...
    exit(header("location:user.php?{$url}"));
}

$info = query("SELECT * FROM ".DB_PREFIX."default_user WHERE `user_id` = '{$user_id}' AND `deleted` = 1 LIMIT 1 ");
if (empty($info->row)) {
    setcookie('action_return', json_encode(['title'=>'删除数据失败', 'info'=>'没有找到匹配的数据，删除数据失败'] , JSON_UNESCAPED_UNICODE), time() + 3600 * 24 * 365);
    exit(header("location:user.php?{$url}"));
}

query("UPDATE ".DB_PREFIX."default_user SET `deleted` = 2 WHERE `user_id` = '{$user_id}' ");
