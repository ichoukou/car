...
$result = query("SELECT e.*,du.name,du.openid,du.channel,g.title AS group_name,g.group_id,i.title,r.title AS round_title FROM ".DB_PREFIX."evaluate AS e " .
    ...
    " WHERE e.`type` = {$type} AND e.`deleted` = 1 {$and} ORDER BY e.type,du.group_id,e.round_id,e.info_id ASC");
...
if (!empty($list)) {
    foreach ($list as $lk=>$lv) {
        ...
        $clist[$lv['group_id'].'_'.$lv['channel'].'_'.$lv['round_id'].'_'.$lv['info_id']][] = $lv;
    }
    ...
    if (!empty($clist)) {
        ...
        foreach ($clist as $ck=>$cv) {
            var_dump($cv);
            exit;
            $objPHPExcel->getActiveSheet()->setCellValue('A' . $tindex,  $cv[0]['type_name']);
            $objPHPExcel->getActiveSheet()->setCellValue('B' . $tindex,  $cv[0]['group_name']);
            $objPHPExcel->getActiveSheet()->setCellValue('C' . $tindex,  $channels[$cv[0]['channel']]);
            $objPHPExcel->getActiveSheet()->setCellValue('D' . $tindex,  $cv[0]['round_title']);
            $objPHPExcel->getActiveSheet()->setCellValue('E' . $tindex,  $cv[0]['title']);
