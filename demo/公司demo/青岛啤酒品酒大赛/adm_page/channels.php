...
$list = query("SELECT * FROM ".DB_PREFIX."channels WHERE `deleted` = 1 {$and} ORDER BY channel_id DESC LIMIT {$param['start']},{$param['limit']}");
$count = query("SELECT COUNT(*) total FROM ".DB_PREFIX."channels WHERE `deleted` = 1 {$and} ");

$pagination         = new Pagination();
$pagination->total  = $count->row['total'];
$pagination->page   = $param['page'];
$pagination->limit  = $param['limit'];
$pagination->url    = "channels.php?page={page}{$url}";
...
$search_url = 'channels.php?';

include_once "header.html";
include_once "left.html";
include_once "channels.html";
