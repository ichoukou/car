...
//		}
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => "{$this->data['entrance']}route=Vender/Common/FileManager"  .'&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url
				);
		...
		$data['parent'] =  "{$this->data['entrance']}route=Vender/Common/FileManager" . $url;
		...
		$data['refresh'] = "{$this->data['entrance']}route=Vender/Common/FileManager" . $url;
		...
		$pagination->limit = 16;
		$pagination->url = "{$this->data['entrance']}route=Vender/Common/FileManager" . $url . '&page={page}';

        $data['pagination'] = $pagination->render();

        $this->data = array_merge($this->data , $data);

        L::output(L::view('Common\\FileManager', 'Vender', $this->data));
	}
